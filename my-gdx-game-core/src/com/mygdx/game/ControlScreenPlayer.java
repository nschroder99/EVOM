...
public class ControlScreenPlayer {
	...
	public ControlScreenPlayer(Sprite ps, Sprite gs, String atl, float h, float he, float wi, float x, float y, float ss, float d, float xv, float yv, boolean invinc)  {
		playerSprite = ps;
		gunSprite = gs;
		health = h;
		xPos = x;
...
//        	fixtureDef.filter.maskBits = PLAYER_MASK_BITS;
	...
	public void assessHealth()  {
		...
		ControlScreen.map[1][25] = h1;
		ControlScreen.map[1][26] = h2;
		ControlScreen.map[1][27] = h3;
		ControlScreen.map[2][25] = h4;
		ControlScreen.map[2][26] = h5;
		ControlScreen.map[2][27] = h6;
	...
	public TextureRegion pAnimate(){
		...
		if(aHit){
			...
			if(aM==1)
    			region = (TextureRegion)lHurt.getKeyFrame(ControlScreen.getElapsedTime(), true);
    		else
    			region = (TextureRegion)rHurt.getKeyFrame(ControlScreen.getElapsedTime(), true);
		}
		...
		idleTimer = 0;
    		...
    		dashWaitTimer = 0;
    		if(m==0) 
    			region = (TextureRegion)rDash.getKeyFrame(ControlScreen.getElapsedTime(), true);
    		else
    			region = (TextureRegion)lDash.getKeyFrame(ControlScreen.getElapsedTime(), true);
    	}
	else if(!(body.getLinearVelocity().y < 0.001 && body.getLinearVelocity().y > -0.001)){
		idleTimer = 0;
    		if(aM==0)
    			region = (TextureRegion)rJump.getKeyFrame(ControlScreen.getElapsedTime(), true); //,playerSprite.getX(),playerSprite.getY(),width*1.1f, height*1.1f);
    		else
    			region = (TextureRegion)lJump.getKeyFrame(ControlScreen.getElapsedTime(), true); //,playerSprite.getX(),playerSprite.getY(),width*1.1f, height*1.1f);
    	}
    	else if(body.getLinearVelocity().x < -0.001 || body.getLinearVelocity().x > 0.001){
    		idleTimer = 0;
    		animTime = 0;
    		if(aM==0) {
    			region = (TextureRegion)rWalking.getKeyFrame(ControlScreen.getElapsedTime(), true); //,playerSprite.getX(),playerSprite.getY(),width, height);
    		}else
    			region = (TextureRegion)lWalking.getKeyFrame(ControlScreen.getElapsedTime(), true); //playerSprite.getX(),playerSprite.getY(),width, height);
    	}
    	else if (!((body.getLinearVelocity().x < -0.001 || body.getLinearVelocity().x > 0.001))){
    		if(idleTimer >= 400){
	    		...
	    		if(aM==0)
	    			region = (TextureRegion)rStanding.getKeyFrame(ControlScreen.getElapsedTime(), true); //,playerSprite.getX(),playerSprite.getY(),width, height);
	    		else
	    			region = (TextureRegion)lStanding.getKeyFrame(ControlScreen.getElapsedTime(), true); //,playerSprite.getX(),playerSprite.getY(),width, height);
	    		idleTimer +=1;
    		}
    	}
    	dashWaitTimer++;
