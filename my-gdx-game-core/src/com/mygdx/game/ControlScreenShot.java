...
public class ControlScreenShot {
	...
	public ControlScreenShot(float x, float y, float xSp, float ySp,float tD, float s,int h,int w,float a,String sA, String aLeft, String aRight, short group){
		xPos = x;
		yPos = y;
		xSpeed = xSp;
		ySpeed = ySp;
...
//        fixtureDef.filter.maskBits = SHOT_MASK_BITS;
	...
	public void update(){
	 ...
	 public void createPShot(int counter){
	    	body = ControlScreen.getWorld().createBody(bodyDef);
	 ...
	 public TextureRegion sAnimate(){
		...
		if(xSpeed <0)
			region = (TextureRegion)lAnimation.getKeyFrame(ControlScreen.elapsedTime, true);
		else
			region = (TextureRegion)rAnimation.getKeyFrame(ControlScreen.elapsedTime, true);
    	update();
    	if(dead){
    		ControlScreen.pShots.remove(this);
    		ControlScreen.getWorld().destroyBody(body);
    	}
    	return region;
     }
